...
const StudentDashboard = () => {
  ...
  useEffect(() => {
    ...
    if (!isAuthenticated || userRole !== 'student') {
      ...
      // Initialize Google Translate with proper loading check
      if (isGoogleTranslateReady()) {
        const savedLanguage = getCurrentLanguage();
        if (savedLanguage && savedLanguage !== 'en') {
          applyGoogleTranslate(savedLanguage);
          // Hide the translate bar after applying translation
          removeTranslateBar();
        }
      } else {
        ...
        window.addEventListener('google-translate-ready', handleTranslateReady);
        
        return () => {
          window.removeEventListener('google-translate-ready', handleTranslateReady);
        };
      }
    }
  }, [navigate]);
